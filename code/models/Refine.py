...
class Regularize_and_RefineNet_DPV_upsample(nn.Module):
    '''
    ...
    def __init__(self, input_volume_channels, feature_dim=32, if_normalize=False, up_sample_ratio=None,
                 C0=0, C1=0, C2=0, D=64, upsample_D=False):
        super(Regularize_and_RefineNet_DPV_upsample, self).__init__()
    ...
    def forward(self, input_volume, img_features):
        '''
        ...
        # res_volume = self.classify(cost1)
        cost0 = self.dres0(input_volume)
        cost1 = self.dres1(cost0) + cost0
        ...
        dpv_raw = torch.squeeze(res_volume, dim=1)
        dpv_raw = F.log_softmax(dpv_raw, dim=1)
...
class RefineNet_DPV_upsample(nn.Module):
    ...
    def __init__(self, C0, C1, C2, D=64, upsample_D=False):
        ...
        # self.apply(self.weight_init) #TODO
