...
class KV_NET_BASIC(nn.Module):
    ...
    def __init__(self, input_volume_channels, feature_dim=32, if_normalize=False, up_sample_ratio=None):
        '''
        inputs:
...
class D_NET_BASIC(nn.Module):
    ...
    def forward(self, ref_frame, src_frames, src_cam_poses, cam_intrinsics=None, BV_predict=None, debug_ipdb=False):
        ...
        Inputs
            ...
            src_frames - N V C H W: V - # of source views, N - usually 1
            src_cam_poses - N V 4 4 - relative cam poses, N = 1

        [relative_extM_src0, ...  ]
        BV_predict - N D H W tensor, the predicted BV, from the last reference frame
        ...
        if self.use_img_intensity:
            ...
            feat_imgs_src = torch.cat((feat_imgs_src, img_int_feats_src), dim=2)
        ...
        costV = warp_homo.est_swp_volume_v4(
            feat_img_ref,  # [1, D+3, H/4, W/4]
            feat_imgs_src,  # [1, 4, D+3, H/4, W/4]
        ...
        if BV_predict is not None:
            print("***** DEBUG! BV_predict is not None")
            import pdb;
            pdb.set_trace()
            # Filtering Framework #
