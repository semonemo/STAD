...
class KVNET(nn.Module):
    ...
    * KV-Net 
    ...
    def forward(self, ref_frame, src_frames, src_cam_poses, BatchIdx=None, cam_intrinsics=None, BV_predict=None, mGPU=False,
                IntMs=None, unit_ray_Ms_2D=None):
        r'''
        Inputs: 
        ref_frame - N C H W format tensor on GPU, N = 1
        src_frames - N V C H W: V - # of source views, N = 1
        src_cam_poses - N V 4 4 - relative cam poses, N = 1
        ...
        BV_predict - N D H W tensor, the predicted BV, from the last reference frame, N=1
        ...
        if (self.if_refined is False) or (self.if_refined is True and self.refineNet_name != 'DPV'):
            ...
            # BV_cur.shape: [1, D, H/4, W/4]
            # d_net_features[0], [1], [2] : [1, D, H/4, W/4], [1, D/2, H/2, W/2], [1, 3, H, W]
        ...
        if not isinstance(BV_predict, torch.Tensor):
            ...
            if mGPU:
                WAPRED_src_frames = warp_homo.warp_img_feats_mgpu(src_frames_dw, self.d_candi, Rs_src, ts_src, IntMs,
                                                                  unit_ray_Ms_2D)
            else:
