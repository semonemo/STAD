...
def get_paths(traj_indx, database_path_base='/datasets/kitti', split_txt=None, mode='train'):
    ...
    scene_names = _read_split_file(split_txt)#[0:2] #TODO
...
class KITTI_dataset(data.Dataset):
    ...
    def __getitem__(self, indx):
        ...
        if dmap_raw is not -1:
            ...
            # dmap_mask = (1 - (proc_totensor(dmap_mask) > 0).float()).bool()
            # dmap_mask_imgsize = (1 - (proc_totensor(dmap_mask_imgsize) > 0).float()).bool()

            dmap_mask = (1 - (proc_totensor(dmap_mask) > 0).float()) > 0.5
            dmap_mask_imgsize = (1 - (proc_totensor(dmap_mask_imgsize) > 0).float()) > 0.5
