...
def test(model_KV, d_candi, Cam_Intrinsics, t_win_r,
    ...
    with torch.no_grad():
        dmap_cur_refined, dmap_refined, d_dpv, kv_dpv = \
            model_KV(ref_frame=ref_frame, src_frames=src_frames, src_cam_poses=Src_CamPoses,
                     BV_predict=BV_predict, mGPU=False, BatchIdx=BatchIdx_range, cam_intrinsics=Cam_Intrinsics)
...
def test_no_pred_DPV(model_KV, d_candi, Cam_Intrinsics, t_win_r,
                     Ref_Dats, Src_Dats, Src_CamPoses, BV_predict,
                     cam_pose_next=None, R_net=False,
                     Cam_Intrinsics_imgsize=None, ref_indx=None):
    '''
    ...
    with torch.no_grad():
        dmap_cur_refined, dmap_refined, d_dpv, kv_dpv = \
            model_KV(ref_frame=ref_frame, src_frames=src_frames, src_cam_poses=Src_CamPoses,
                     mGPU=False, BatchIdx=BatchIdx, cam_intrinsics=Cam_Intrinsics)


    # with torch.no_grad():
    #     for ibatch in range(ref_frame.shape[0]):
    #         dmap_cur_refined, dmap_refined, d_dpv, kv_dpv = model_KV(ref_frame=ref_frame[ibatch].unsqueeze(0),
    #                                                                  src_frames=src_frames[ibatch].unsqueeze(0),
    #                                                                  src_cam_poses=Src_CamPoses[ibatch].unsqueeze(0),
    #                                                                  cam_intrinsics=Cam_Intrinsics[ibatch],
    #                                                                  BatchIdx=BatchIdx
    #                                                                  )
    if R_net:
        return dmap_refined, None
    else:
        return kv_dpv, None
