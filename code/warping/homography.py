...
def warp_img_feats_mgpu(feat_img_src, d_candi, R, t, IntM_tensors, unit_ray_arrays_2D):
    ...
    if isinstance(R, list) and isinstance(t, list):
        ...
        for idx_view, feat_img_src_view in enumerate(feat_img_src):
            ...
            feat_img_src_view_warp_par_d = _back_warp_homo_parallel_v1(feat_img_src_view_repeat, d_candi_cuda, term1,
                                                                       term2, IntM_tensor, H, W)
...
def warp_img_feats_v3(feat_img_src, d_candi, R, t, cam_intrinsic):
    ...
    if isinstance(R, list) and isinstance(t, list):
        ...
        for idx_view, feat_img_src_view in enumerate(feat_img_src):
            ...
            term2 = IntM_tensor.matmul(R[idx_view]).matmul(P_ref_cuda)
            feat_img_src_view_repeat = feat_img_src_view.repeat(len(d_candi), 1, 1, 1)
            feat_img_src_view_warp_par_d = _back_warp_homo_parallel(feat_img_src_view_repeat, d_candi_cuda, term1,
                                                                    term2, cam_intrinsic, H, W)
            Feat_warped_src.append(torch.transpose(feat_img_src_view_warp_par_d, dim0=0, dim1=1))
...
def est_swp_volume_v4(feat_img_ref, feat_img_src, d_candi, R, t, cam_intrinsic,
                      costV_sigma, feat_dist='L2', debug_ipdb=False):
    r'''
    feat_img_ref - N C H W tensor
    feat_img_src - N V C H W tensor.  V is for different views
    ...
    P_ref_cuda = cam_intrinsic['unit_ray_array_2D'].cuda(
        torch.cuda.current_device())  # unit ray array in matrix form on GPU
    ...
    for idx_view in range(feat_img_src.shape[1]):
        ...
        # Get term2 #
        term2 = IntM_tensor.matmul(R[idx_view, :, :]).matmul(P_ref_cuda)
        feat_img_src_view = feat_img_src[:, idx_view, :, :, :]  # (1,D+3,H,W)
        feat_img_src_view_repeat = feat_img_src_view.repeat(len(d_candi), 1, 1, 1)  # (D,D+3,H,W)

        # (D,D+3,H,W)
        # feat_img_src_view_warp = temp_back_warp_homo_parallel(feat_img_src_view, d_candi_cuda, term1, term2, cam_intrinsic, H, W)
        feat_img_src_view_warp_par_d = _back_warp_homo_parallel(feat_img_src_view_repeat, d_candi_cuda, term1, term2,
                                                                cam_intrinsic, H, W)
        ...
        if feat_dist == 'L2':
            ...
            costV[0, :, :, :] = costV[0, :, :, :] + img_dis_L1_pard(feat_img_src_view_warp_par_d,
                                                                    feat_img_ref) / costV_sigma
        else:
...
def resample_vol_cuda(src_vol, rel_extM, cam_intrinsic=None, d_candi=None, d_candi_new=None,
                      padding_value=0., is_debug=False, PointsDs_ref_cam_coord_in=None):
    r'''
    ...
    if PointsDs_ref_cam_coord_in is None:
        ...
        for idx_d, d in enumerate(d_candi_):
            PointsDs_ref_cam_coord[0, idx_d, :, :, :] = d * torch.FloatTensor(cam_intrinsic['unit_ray_array'])
        PointsDs_ref_cam_coord = PointsDs_ref_cam_coord.cuda(rel_extM.get_device())
    else:
    ...
    PointsDs_ref_cam_coord = torch.cat(
        (PointsDs_ref_cam_coord, torch.ones(1, PointsDs_ref_cam_coord.shape[1]).cuda(rel_extM.get_device())), dim=0)

    src_cam_extM = rel_extM
    PointsDs_src_cam_coord = src_cam_extM.matmul(PointsDs_ref_cam_coord.cuda(src_cam_extM.get_device()))

    # transform into range [-1, 1] for all dimensions #
    PointsDs_src_cam_coord[0, :] = PointsDs_src_cam_coord[0, :] / (PointsDs_src_cam_coord[2, :] + 1e-10) / math.tan(
        hhfov)
    PointsDs_src_cam_coord[1, :] = PointsDs_src_cam_coord[1, :] / (PointsDs_src_cam_coord[2, :] + 1e-10) / math.tan(
        hvfov)
    ...
    res_vol_th = torch.squeeze(
        torch.squeeze(F.grid_sample(src_vol_th_, PointsDs_src_cam_coord, mode='bilinear', padding_mode='border'),
                      dim=0),
        dim=0)
