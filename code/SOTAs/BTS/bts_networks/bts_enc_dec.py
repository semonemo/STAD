...
class silog_loss(nn.Module):
    def __init__(self, variance_focus, weighted_sd=0):
        super(silog_loss, self).__init__()
        self.variance_focus = variance_focus
        self.weighted_sd = weighted_sd

    def forward(self, depth_est, depth_gt, mask):
        d = torch.log(depth_est[mask]) - torch.log(depth_gt[mask])
        loss = torch.sqrt((d ** 2).mean() - self.variance_focus * (d.mean() ** 2)) * 10.0
        if loss != loss or loss == 0:
            print(f'occured nan loss! diff.max={d.max().item()} diff.min={d.min().item()}')
            loss = 0
        return loss

    def forward_wieghted_sd(self, depth_est, depth_gt, mask, sd_mask):
        weighted_mask = (mask.float() + sd_mask.float() * self.weighted_sd)[mask]
        d = (torch.log(depth_est[mask]) - torch.log(depth_gt[mask])) * weighted_mask
        loss = torch.sqrt((d ** 2).mean() - self.variance_focus * (d.mean() ** 2)) * 10.0
        if loss != loss or loss == 0:
            print(f'occured nan loss! diff.max={d.max().item()} diff.min={d.min().item()}')
            loss = 0
    ...
    def forward_sqrtX(self, depth_est, depth_gt, mask):
        d = torch.log(depth_est[mask]) - torch.log(depth_gt[mask])
...
class bts(nn.Module):
    def __init__(self, params, feat_out_channels, num_features=512):
        ...
        self.daspp_conv = torch.nn.Sequential(
            ...
            nn.ELU())
        '''
        self.corr_layer = torch.nn.Sequential(
            nn.BatchNorm2d(num_features // 4, momentum=0.01, affine=True, eps=1.1e-5),
            nn.Conv2d(num_features // 4, num_features // 4, 1, 1, 0, bias=False),
            nn.ELU(),
            nn.BatchNorm2d(num_features // 4, momentum=0.01, affine=True, eps=1.1e-5),
            nn.Conv2d(num_features // 4, num_features // 4 * 2, 1, 1, 0, bias=False),
            nn.ELU(),
            nn.BatchNorm2d(num_features // 4 * 2, momentum=0.01, affine=True, eps=1.1e-5),
            nn.Conv2d(num_features // 4 * 2, num_features // 4, 1, 1, 0, bias=False),
            nn.ELU()
        )
	'''
    ...
    def forward_step1(self, features):
        ## features[0]       : [1, 3, 352, 1216]; H
        # features[1] (skip0): [1, 96, 176, 608]; H/2
        # features[2] (skip1): [1, 96,  88, 304]; H/4
        # features[3] (skip2): [1, 192, 44, 152]; H/8
        # features[4] (skip3): [1, 384, 22,  76]; H/16
        # features[5]        : [1, 2208, 11, 38]; H/32
        skip0, skip1, skip2, skip3 = features[1], features[2], features[3], features[4]
        dense_features = torch.nn.ReLU()(features[5])  # [1, 2208, 11, 38]
    ...
    def forward_step2(self, features, focal, iconv5):
        ...
        iconv4 = self.bn4_2(iconv4)  # [1, 256, 44, 152]; H/8
        ...
        reduc8x8 = self.reduc8x8(daspp_feat)  # [b, 128, 44, 152]=>[b, 4, 44, 152]
        plane_normal_8x8 = reduc8x8[:, :3, :, :]
        plane_normal_8x8 = torch_nn_func.normalize(plane_normal_8x8, 2, 1)
        plane_dist_8x8 = reduc8x8[:, 3, :, :]
        plane_eq_8x8 = torch.cat([plane_normal_8x8, plane_dist_8x8.unsqueeze(1)], 1)  # [1, 4, 44, 152])
        depth_8x8 = self.lpg8x8(plane_eq_8x8, focal)  # [1, 352, 1216]
        depth_8x8_scaled = depth_8x8.unsqueeze(1) / self.params.max_depth
        depth_8x8_scaled_ds = torch_nn_func.interpolate(depth_8x8_scaled, scale_factor=0.25,
                                                        mode='nearest')  # [1, 1, 88, 304]

        upconv3 = self.upconv3(daspp_feat)  # H/4
        upconv3 = self.bn3(upconv3)
        concat3 = torch.cat([upconv3, skip1, depth_8x8_scaled_ds], dim=1)
        iconv3 = self.conv3(concat3)  # [b, 128, 88, 304]

        reduc4x4 = self.reduc4x4(iconv3)  # [1, 128, 88, 304] -> [1, 4, 88, 304]
        ...
        upconv2 = self.bn2(upconv2)  # [1, 64, 176, 352]
        concat2 = torch.cat([upconv2, skip0, depth_4x4_scaled_ds], dim=1)
        iconv2 = self.conv2(concat2)  # [b, 64, 176, 608]
        ...
        upconv1 = self.upconv1(iconv2)  # ([b, 32, 352, 704])
        reduc1x1 = self.reduc1x1(upconv1)  # ([b, 1, 352, 704])
        concat1 = torch.cat([upconv1, reduc1x1, depth_2x2_scaled, depth_4x4_scaled, depth_8x8_scaled], dim=1)
        iconv1 = self.conv1(concat1)  # [b, 32, 352, 704]
    ...
    def forward_step1_daspp(self, features):
        ## features[0]       : [1, 3, 352, 1216]; H
        # features[1] (skip0): [1, 96, 176, 608]; H/2
        # features[2] (skip1): [1, 96,  88, 304]; H/4
        # features[3] (skip2): [1, 192, 44, 152]; H/8
    ...
    def forward_step2_daspp(self, features, focal, daspp_feat):
        skip0, skip1, skip2, skip3 = features[1], features[2], features[3], features[4]
        ######################
        # daspp_feat = self.corr_layer(daspp_feat)
