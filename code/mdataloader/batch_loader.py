    ...
    def _get_traj_lengths(self):
        ...
        for ibatch in range(self.batch_size):
            if raw_traj_batch_len[ibatch] == traj_len + 2 * self.t_win_r:
                ...
                trajs_st_frame[ibatch] = int(
                    random.randrange(self.t_win_r, raw_traj_batch_len[ibatch] - traj_len - t_win_r))
            else:
                trajs_st_frame[ibatch] = self.t_win_r
    ...
    def local_info(self):
        ...
        for ibatch in range(self.batch_size):
            ...
            if is_valid_:
                ...
                src_cam_pose_ = [warp_homo.get_rel_extrinsicM(ref_dat_['extM'], src_cam_extM_) for src_cam_extM_ in
                                 src_cam_extMs]
